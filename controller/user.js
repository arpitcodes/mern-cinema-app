...
exports.signUp = (req, res) => {
    ...
    User.findOne({ email })//check weather email exists 
        .then((user) => {
            if (user)
                res.status(409).json({
                    error: "The entered Email already exist!"
                });
            else {
                //hash the password
                bcrypt.hash(password, 10, (error, hash) => {
                    if (error) res.status(500).json({ error });
                        ...
                        userData.save().then(() => {
                            //create resusable transporter object using the defaultl smtp transport
                            let transport = nodemailer.createTransport({
                                service: "gmail",
                                port: 587,
                                secure: false, //true for 465 false for other ports
                                auth: {
                                    user: process.env.SENDER_EMAIL,//generate ethereal user
                            ...
                            trasporter.sendMail({
                                ...
                                html: `<b>Hello Dear User, we are happy that you join our family. kind Regards, Cinema App Team.</b>`,
        ...
        if (err || user.length === 0)
            ...
            bcrypt.compare(password, user[0].password, (_err, result) => {
                if (_err) res.status(401).json({ error: "Authentication has failed!" });
                    ...
                    const token = jwt.sign(userData, "MONGO_SECRET", {
                        expiresIn: "1h"
                    });
                    res.status(200).json({
                        message: "Authentication has been successfull",
                        token: token,
                        userData,
                    });
                } else
...
exports.updateUser = (req, res, next) => {
    ...
    User.updateMany({ _id: userID }, { $set: req.body }).then((result) => result.state(200).json(result)).catch((error) => res.status(409).json(error));
}
